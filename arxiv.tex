...
\input{macros}
\usepackage{multibib}
\newcites{Str}{My Publications \small{\textcolor{purple}{$(\alpha\beta)$alphabetic order}, \textcolor{red}{$^\star$}first  author contribution, }}
...
\maketitle
\begin{abstract}
We challenge the common wisdom that \emph{reward is enough} to \emph{learn} intelligent behavior. In many practical settings---robotics, LLM fine-tuning, multi-agent systems---designing a correct reward function is infeasible, and practitioners instead rely on expert demonstrations or human preferences. Yet existing imitation learning (IL) and alignment algorithms rest on unrealistic assumptions. This thesis removes four of them, yielding the following contributions.
\begin{enumerate}
    \item \textbf{Multi-agent IL.} Standard IL algorithms are restricted to single-agent systems and fail arbitrarily in games. We characterize the statistical complexity of learning Nash equilibria from demonstrations and introduce RMAX-RAVI-UCB-ZERO-BC, the first rate-optimal IL algorithm for Markov games.
    \item \textbf{IL from features.} Existing IL requires observing expert \emph{actions}, but recordings (e.g., video) reveal only features. We prove that environment interaction is necessary in this setting and design FRA-IL, the first min-max optimal algorithm for IL from features in Linear MDPs.
    \item \textbf{IL without expert realizability.} Standard IL guarantees require the learner's policy class to contain the expert policy, which fails in distillation or when learning from a privileged expert. We introduce SPOIL and OVI, the first offline and interactive IL algorithms whose sample complexity depends only on the complexity of the \emph{Q-value} class, not the expert's policy.
    \item \textbf{Robust alignment.} DPO and its variants avoid overfitting only when the data-generating policy is known, which rarely holds in practice. We propose PEPO, the first alignment algorithm that provably avoids over-optimization without any knowledge of the data-generating policy.
\end{enumerate}
The theoretical results are validated experimentally in simulated robotics and in post-training of LLMs up to 34B parameters.
\end{abstract}
...
\item \textbf{Limitation 3:} Existing IL algorithms apply supervised learning but this only works when the algorithm uses a network
 ...
 \item \textbf{Limitation 4:} Finally, looking at alignment algorithms we notice that common choices such as DPO overfit, and alternatives such as $\chi^2$PO and DPO-SFT fix the problems only when the
...
In finance, we can think of the state as the current budget of an AI automated investor and as action space the possible 
...
\begin{example}
\textbf{LLM post-training.} In the context of LLM, $\piref$ is a model skilled in tasks that can be well-learned by next-token prediction. For example, producing grammatically correct sentences.
In contrast $r^\star$ rewards highly behaviours that requires reasoning, i.e. producing a correct math proof. The objective we consider above aims at finding a tradeoff between these skills.
\end{example}
\subsection{Folklore imitation learning.}
The standard and most common approach to imitation learning and alignment is to implement the maximum likelihood principle. 
In imitation learning, the folklore approach started in \citep{Pomerleau:1991} is to maximize the likelihood of the observed expert actions, while in alignment
...
under the Bradley-Terry model \citep{bradley1952rank}.
 In particular, denoting the state-action dataset sampled from the expert occupancy measure as $\bc{\XEi,\AEi} \sim \mu^{\expert}$ be the expert dataset to be imitated we can consider $\XEi$ as the covariates and $\AEi$ as the labels to be predicted. This point of view motivates the following algorithm,
\[
...
Finally, nothing is known about the behavior of BC in multi-agent systems. We show that BC is suboptimal in that settings and we introduce an alternative algorithm that does not suffer \textbf{Limitation 1}.
...
$\gA^1,\gA^2, \dots, \gA^n$,and the transition kernel depends now on all players' actions  $P:\X\times\A\rightarrow \Delta_{\X}$. In other words, $P(x' \mid x,a^1,a^2, \dots, a^n)$ denotes the probability of landing in $x'$ from $x$ under the action profile $a^1,a^2,\dots, a^n$. The reward function for the $i^{\mathrm{th}}$ player also depends on the whole action profile
...
\begin{theorem} \textbf{Behavioral cloning fails in games \citeStr{freihaut2025learning}}
There exists a Markov Game and a Nash equilibrium profile, such that the output $\piout$ of BC or any other offline IL algorithm is
arbitrarily far from being a Nash Equilibrium. That is,
...
\end{theorem}
\end{tcolorbox}
Moreover, our later work \citeStr{freihaut2026rate} that, even when BC converges, its rate is suboptimal.
Next, we present a new algorithm named RMAX-RAVI-UCB-ZERO-BC that is guaranteed to learn a $\varepsilon$-Nash equilibrium in any game. Key for the success is to allow a more powerful access model to the expert.
...
Intuitively this means that we do not only observe strong players playing among them but we can play according to a policy of our choice against the other player's strong policy. 
This allows to observe how the opponents policies behave in states that would never be encountered if all players were following a Nash profile.
\begin{tcolorbox}[
    colback=orange!10!white,
    colframe=orange!80!black,
    %boxrule=0.8pt,
...
\begin{theorem} \textbf{RMAX-RAVI-UCB-ZERO-BC \citeStr{viano2026multi} learns in any game.}
    RMAX-RAVI-UCB-ZERO-BC outputs a policy $\piout$ which is a $\varepsilon$-Nash Equilibrium with probability at least $1-\delta$ after 
    $\mathcal{O}\br{\log(1/\delta) \varepsilon^{-2}}$ expert samples.
\end{theorem}
\end{tcolorbox}
The idea behind the design of RMAX-RAVI-UCB-ZERO-BC is to first design a data collection phase 
which plays against the Nash profile to collect an exploratory dataset. This is implemented running our COLT algorithm RMAX-RAVI-UCB \citepStr{moulin2025optimistically} to maximize an exploratory reward.
Secondly, this dataset is used for BC to output the learned policy.
Our algorithms are not only enjoying better theoretical properties but also lead to improved performance in distillation for games. 
...
This enables the use of massively larger dataset containing only the states of the expert or more generally only expert features.
...
a dataset which is considerably less informative than $\bc{X^i_E, A^i_E}^{\tau_E}_{i=1}$ when the feature map is information-lossy as it happens for example when $\phi_r$ represents a video frame.
Our first fundamental contribution is that, because of the information loss, it is no longer possible to learn 
without access to the environment to estimate the transition dynamics.
...
\begin{theorem} \textbf{Imitation Learning from features requires environment access. \citepStr{moulin2025optimistically}}
...
dubbed FRA-IL \citep{moulin2025optimistically} when the transition dynamics are also linear in some other feature map.
...
Finding the right updates for the policy player, solved a problem open in RL theory since 5 years \citep{jin2019provably} which consisted of designing no-regret algorithms for infinite horizon MDPs with linear transitions and rewards.
...
\begin{theorem} \textbf{Rate optimal upper bounds for Imitation Learning from features.\citepStr{moulin2025optimistically}}
...
Moreover, the bounds can not be improved in terms of $\varepsilon$ because of our lower bound. Therefore, FRA-IL is rate optimal.

\paragraph{Practical take-away}
From the above theoretical analysis, we learn that exploration in the policy update is crucial. In stark contrast, existing algorithms were neglecting this aspect.
We therefore designed, inspired by theory above, a meta algorithm \citeStr{viel2025soar} that injects exploration in existing algorithms to learn only from features in simulated robotics.
...
In some situations the expert might choose actions at a given state using external information which is not revealed to the learner, we call this a privileged expert.
...
or information acquired elsewhere. Without access to such privileges, the learner can not hope to match 
the expert distribution. However, there is still hope to perform similarly in terms of gained money, even though by taking different actions.
\end{example}
In our NeurIPS 2025 paper, \citepStr{moulin2025inverse} we invent the first offline imitation learning which does not mimic the expert.
...
then the algorithm SPOIL \citepStr{moulin2025inverse} learns a $\varepsilon$-optimal policy observing $\cO(\varepsilon^{-2})$ offline expert data, without any dependence on $\abs{\PiE}$.    
At the same time, if $\cQ$ only satisfies \textbf{Case II}, no algorithm can learn.
\end{theorem}
\end{tcolorbox}
The above theorem gives a positive result under \textbf{Case I}, but how to learn under the weaker assumption (\textbf{Case II}) ?
...
becomes stronger and stronger compared to the maximum likelihood-based approaches (BC and DAgger).
...
In the last chapter of my thesis, based on \citeStr{barla2026provably}, I remove this limitation with a new algorithm: PEPO.
...
\end{theorem}
\end{tcolorbox}
Notably, in the above theorem there is no dependence at all on $C^{\mathrm{all}}$ and the dependence on $C^\star$ is better than what obtained in prior works. 
...
It can also be noticed that the ensemble size $L$ has a strong effect on the performance. Our theory provides 
guidance for the optimal choice. In particular, if $L$ is too small, then we cannot guarantee
...
This work studied principled algorithms for learning good policies in decision-making problems when a reward function is not available \emph{as a learning signal}. Our fundamental contributions are: 

\begin{itemize}
\item~a precise characterization of the statistical complexity for learning equilibria in Markov games from data, with the first rate-optimal algorithm (RMAX-RAVI-UCB-ZERO-BC); 
\item~the first min-max optimal and computationally efficient algorithm for IL from features in Linear MDPs (FRA-IL); 
\item~the first IL algorithms that learn \emph{without requiring expert realizability} (SPOIL/OVI); and 
\item~the first DPO-like algorithm provably avoiding over-optimization without any knowledge of the data-generating policy (PEPO).
\end{itemize}
Despite these exciting results, several important directions remain open.

\textbf{Beyond expected reward.}
The suboptimality metric studied here is based on expected reward. However, small expected suboptimality does not exclude dangerous actions occurring with non-negligible probability. The uncertainty quantification techniques we developed for optimistic exploration (FRA-IL) and pessimistic regularization (PEPO) could be repurposed to detect high-risk situations and force the agent to abstain from acting. Designing no-regret abstention algorithms where the abstention cost reflects the missed opportunity is well studied in the bandit setting \citep{cortes2018online,neu2020fast,pasteris2024bandits}, but remains largely unexplored in multi-step decision-making, despite its clear practical relevance---as witnessed by benchmarks evaluating the abstention capability of modern LLMs \citep{fan2026halluhard}.

\textbf{Coverage and test-time compute.}
Expected reward also fails to capture the probability mass placed on the set of optimal actions (coverage \citep{chen2025coverage,wagenmaker2025posterior}). In modern LLM pipelines---where IL and alignment are followed by a test-time compute phase, often implemented via Best of N (BoN) \citep{gui2024bonbon,huang2025best}---coverage rather than expected reward determines how effectively BoN improves quality. Extending the guarantees of this thesis to provide lower bounds on the success probability of BoN sampling from the output of an IL or alignment algorithm is therefore an important open problem.

All in all, the take-away is that reward alone \emph{is not enough} to guide the design of complex modern decision-making systems, particularly in LLMs and robotics. The developments in my thesis bypassed several limitations of existing works, however solving the missing steps listed in the conclusions still requires some steps of an exciting research journey.
...
\bibliographystyleStr{plain}
\bibliographyStr{refs1}

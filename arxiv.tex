...
\end{example}
In our NeurIPS 2025 paper, \citep{moulin2025inverseqlearningrightoffline} we invent the first offline imitation learning which does not mimic the expert.
...
This work studied principled algorithms for learning good policies in decision-making problems when a reward function is not available \emph{as a learning signal}. Our fundamental contributions are: (i)~a precise characterization of the statistical complexity for learning equilibria in Markov games from data, with the first rate-optimal algorithm (RMAX-RAVI-UCB-ZERO-BC); (ii)~the first min-max optimal and computationally efficient algorithm for IL from features in Linear MDPs (FRA-IL); (iii)~the first offline IL algorithm that learns \emph{without requiring expert realizability} (SPOIL/OVI); and (iv)~the first DPO-like algorithm provably avoiding over-optimization without any knowledge of the data-generating policy (PEPO).
...
Expected reward also fails to capture the probability mass placed on the set of optimal actions (coverage \citep{chen2025coverage,wagenmaker2025posterior}). In modern LLM pipelines---where IL and alignment are followed by a test-time compute phase via Best of N (BoN) \citep{gui2024bonbon,huang2025best}---coverage rather than expected reward determines how effectively BoN improves quality. Extending the guarantees of this thesis to provide lower bounds on the success probability of BoN sampling from the output of an IL or alignment algorithm is therefore an important open problem.

\textbf{Continuous action spaces.}
IL is widely applied in robotics where the action space is continuous, yet our computational efficiency guarantees crucially rely on finite action spaces---a limitation shared by recent IL theory works \citep{foster2024behavior,rohatgi2025computational,joshi2025learning}. Action quantization \citep{dadashi2021continuous} and environment interaction \citep{viano2022proximal,viano2024imitation,moulin2025optimistically} offer promising paths beyond this limitation.

All in all, we believe that reward alone \emph{is not enough} to guide the design of complex modern decision-making systems, particularly in LLMs and robotics. The results presented here and in today's growing literature are, we believe, just the first steps of an exciting journey.
